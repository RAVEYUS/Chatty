...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const requestAlreadySent = await ctx.db
            .query("requests")
            .withIndex("by_receiver_sender", (q) => q.eq("receiver", receiver._id).eq("sender", currentUser._id)).unique();

        if (requestAlreadySent) {
            throw new ConvexError("Request Already Sent");
        }

        const requestAlreadyReceiver = await
            ctx.db
                .query("requests")
                .withIndex("by_receiver_sender", (q) => q.eq("receiver", currentUser._id).eq("sender", receiver._id)).unique();
...
export const deny = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!request || request.receiver !== currentUser._id) {
            throw new ConvexError("There was a error denying this request");
        }
